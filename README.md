...
👋 안녕하세요, 혁신을 사랑하는 여러분! 🌟  
<br><br>
우리는 <b>런웨이팀</b>, 세상을 따뜻하게 바꾸는 <b>스마트 신발 개발자</b>입니다. 👣💡  
<br><br>
...
| **소프트웨어** | **하드웨어** |
|----------------|--------------|
| • 이탈 시 관리자(요양보호사)가 대시보드를 통한 이탈 감지 <br> • 배회 치매노인의 신고 및 추적을 위해 메일을 통한 사진 전송(보호자, 경찰, 요양기관) | • 카메라 : 외출 감지 및 촬영 <br> • BLE 비콘 : 주파수 발신(저전력, 소형화) <br> • 라즈베리파이 : 주파수 수신 및 감지, 거리측정, 기록분석, 알람 등 제공 |
...
| **기대효과** | **활용분야** |
|---------------|--------------|
| • **치매 어르신에게 거부감이 없는 디자인과 기능 설계**로 실효성 높은 제품 구현 <br> • **출입 및 위험 움직임 실시간 감지**로 보호자 대응력 향상 <br> • **저전력·교체형 구조**로 유지관리 편의성 및 시장 경쟁력 확보 | • **소형화 모듈 기반으로 스마트 신발 시장 확대 가능성** <br> • **아동·장애인 등 이동 취약계층의 이탈 방지 서비스 확대 적용 가능** <br> • **지자체·복지기관 연계형 광역 돌봄 체계 구축**으로 지역 사회 안전망 강화 |
...
|:---:|:---:|:---:|:---:|:---:|
| **이유림** | **엄지수** | **이화연** | **이효빈** | **홍유민** |
| • 프로젝트 총괄 <br> • 시스템 기능 통합 조정 및 시연 영상 제작 | • 프론트엔드 개발 <br> • 사용자 인터뷰·요구사항 분석 | • 하드웨어 및 카메라 제어 담당 <br> • 시각자료 제작 |• 디자인 총괄 <br> • 데이터 전송 및 이메일 알림 모듈 개발 | • 백엔드 개발 (Flask 서버) <br> • 시연 담당 |
...
<img width="600" height="563" alt="image" src="https://github.com/user-attachments/assets/cfd224ca-dcc1-4a01-bd99-49b0feb1dd5a" />


## ⚙️ 시스템 구성도

본 시스템은 **하드웨어**와 **소프트웨어**의 연동을 통해  
치매 어르신의 외출 감지와 보호자 알림 기능을 수행합니다.

### 🧩 하드웨어 구성
라즈베리파이를 중심으로 카메라, BLE, Wi-Fi, 저장장치(SD카드)가 연결되어 있습니다.  
- **Camera** : 외출 시 자동 촬영  
- **BLE 모듈** : 비콘 신호 송수신 및 거리 감지  
- **Wi-Fi / SD** : 데이터 저장 및 전송  
- **알림** : BLE 기반 알림 수신

### 💻 소프트웨어 구성
Python 기반으로 작성된 프로그램이 센서 데이터를 처리하고  
MySQL 데이터베이스 및 서비스(APU)를 통해  
사진 전송, 알림 등 주요 기능을 제공합니다.  

> 하드웨어에서 수집된 정보는 라즈베리파이를 통해 관리자 대시보드와 보호자에게 실시간으로 전달됩니다.
...
본 단계는 **비콘 수신기(라즈베리파이)** 를 중심으로  
수집된 신호 데이터를 **서버와 웹 애플리케이션**으로 전달하는 구조입니다.

### 🔹 동작 개요
1. **비콘 신호 송신**  
   치매 어르신이 착용한 스마트 신발 내 비콘이 지속적으로 신호를 발신합니다.  
2. **비콘 수신기(라즈베리파이)**  
   주변의 라즈베리파이가 비콘 신호를 수신하고 거리 정보를 계산합니다.  
3. **데이터 전송 및 처리**  
   측정된 데이터가 **Web Server**로 전송되어 저장·분석됩니다.  
4. **관리자 및 보호자 알림**  
   서버는 Python, MySQL, Apache Web 환경에서 작동하며,  
   실시간으로 대시보드 또는 모바일 기기에 **이탈 알림 및 사진 정보**를 제공합니다.


📡 **요약**
> 비콘 → 라즈베리파이(수신기) → Web Server → 보호자·관리자에게 동시 데이터가 전달되어 **실시간 위치 감지 및 대응 체계**를 구현합니다.
